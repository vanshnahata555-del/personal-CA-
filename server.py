...
def gemini_generate(system_instruction: str, contents: list[dict]) -> str:
    if not GEMINI_API_KEY:
        raise HTTPException(503, "Gemini AI is not configured. Add GEMINI_API_KEY to Render Environment Variables.")
    ...
    try:
        ...
        # Keep the API key out of logs/errors while exposing the useful status code.
        raise HTTPException(502, f"Gemini API returned HTTP {exc.code}. Check the Gemini key, model and quota in Google AI Studio.") from exc
    except (urllib.error.URLError, TimeoutError) as exc:
        raise HTTPException(502, "Could not reach the Gemini API. Please try again in a moment.") from exc
...
def ai_chat(body: AIRequest, request: Request):
    ...
    return {"reply": gemini_generate(AI_INSTRUCTIONS, contents)}
...
def stocks_ai_research(body: StockResearchRequest, request: Request):
    ...
    return {
        ...
        "reply": gemini_generate(STOCKS_AI_INSTRUCTIONS, contents),
    }
